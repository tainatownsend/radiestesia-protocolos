...
function allResults(query){
  ...
  (state.tools||[]).filter((t)=>!t.archivedAt).forEach((t)=>push('tool',t.id,t.name,'Biblioteca',`${t.purpose||''} ${t.notes||''}`));
...
function revealResult(kind,id){
  ...
  if(kind==='tool'){route('library');requestAnimationFrame(()=>{const state=store.getState();const name=state.tools.find((t)=>t.id===id)?.name;const card=[...document.querySelectorAll('main article.card')].find((c)=>c.querySelector('h3,h2')?.textContent?.trim()===name);card?.scrollIntoView({behavior:'smooth',block:'center'});card?.classList.add('search-highlight');setTimeout(()=>card?.classList.remove('search-highlight'),1800);});return;}
