...
function pendingItems(state) {
  ...
  for (const investigation of state.investigations || []) {
    ...
    items.push({
      kind:'investigation', investigationId:investigation.id,
  ...
  for (const treatment of state.treatments || []) {
    ...
    items.push({
      kind:'treatment', assistedId:treatment.assistedEntityId,
      title:treatment.title || 'Tratamento', treatmentId:treatment.id,
...
function ensureContinuity() {
  ...
  section.className='section';section.dataset.todayContinuity='true';
  const visible=items.slice(0,5);
  section.innerHTML=`<div class="section-head"><div><p class="eyebrow">Para continuar</p><h2>Trabalhos que ficaram em aberto</h2></div>${items.length>visible.length?`<button class="btn ghost small" data-route="treatments">Ver mais</button>`:''}</div><div class="stack">${visible.map((item)=>`<article class="card continuity-card"><div><p class="eyebrow">${esc(item.label)}</p><h3>${esc(item.title)}</h3><p class="muted">${esc(assistedName(state,item.assistedId))}</p></div>${item.kind==='treatment'?`<button class="btn secondary small" data-route="treatments" data-continuity-treatment="${item.treatmentId}">Abrir</button>`:`<button class="btn secondary small" data-continuity-investigation="${item.investigationId}">Preparar e retomar</button>`}</article>`).join('')}</div>`;
...
function enhance(){if(enhancing)return;enhancing=true;try{ensureContinuity();}finally{enhancing=false;}}
new MutationObserver(enhance).observe(document.body,{childList:true,subtree:true});queueMicrotask(enhance);

document.addEventListener('click',(event)=>{
  const button=event.target.closest('[data-continuity-treatment]');if(!button)return;
  ...
  requestAnimationFrame(()=>{
    ...
    card?.scrollIntoView({behavior:'smooth',block:'center'});card?.classList.add('search-highlight');
