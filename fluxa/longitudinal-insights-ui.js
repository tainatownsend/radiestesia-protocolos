...
function assistedSummary(state,id){
  ...
  const findings=(state.findings||[]).filter((f)=>f.assistedEntityId===id&&f.status!=='DISMISSED').sort((a,b)=>(b.updatedAt||b.createdAt||'').localeCompare(a.updatedAt||a.createdAt||''));
  const assessments=(state.assessments||[]).filter((a)=>a.assistedEntityId===id).sort((a,b)=>(b.occurredAt||b.createdAt||'').localeCompare(a.occurredAt||a.createdAt||''));
  const imbalance=[...assessments].reverse().filter((a)=>Number.isFinite(Number(a.imbalancePercent))).slice(-6);
  const sessionIds=new Set((state.events||[]).filter((e)=>e.assistedEntityId===id&&e.sessionId).map((e)=>e.sessionId));
  const sessions=(state.sessions||[]).filter((s)=>sessionIds.has(s.id)).sort((a,b)=>b.startedAt.localeCompare(a.startedAt));
...
function trendHtml(items){
  ...
  return `<div class="imbalance-trend">${items.map((a)=>{const value=Math.max(0,Math.min(100,Number(a.imbalancePercent)));return `<div class="trend-point"><div class="trend-bar-track"><span style="height:${Math.max(6,value)}%"></span></div><strong>${value}%</strong><small>${fmt(a.occurredAt||a.createdAt)}</small></div>`;}).join('')}</div><p class="muted">Percentual de desequilíbrio registrado nas avaliações finais.</p>`;
}
