...
function ensureSessionHistoryAction() {
  ...
  if (!main || main.querySelector('[data-session-history]')) return;
  if (main.querySelector('.eyebrow')?.textContent?.trim() !== 'Hoje') return;
  const recentHeading = [...main.querySelectorAll('h2')].find((node) => node.textContent?.trim() === 'Atividade recente');
  const section = recentHeading?.closest('.section');
  if (!section) return;
  const button = document.createElement('button');
  button.className = 'btn ghost small';
  button.dataset.sessionHistory = 'true';
  button.textContent = 'Ver sessões';
...
function ensureTreatmentHistoryActions() {
  ...
  document.querySelectorAll('.treatment-card').forEach((card, index) => {
    ...
    const treatment = state.treatments.find((item) => item.id === card.dataset.treatmentId) || treatments[index];
    if (!treatment) return;
    const row = card.querySelector('.button-row') || card.appendChild(Object.assign(document.createElement('div'), { className:'button-row' }));
    const button = document.createElement('button');
    button.className = 'btn ghost small';
    button.dataset.treatmentHistory = treatment.id;
...
function enhance() {
  ...
  enhancing = true;
  try {
    ensureSessionHistoryAction();
    ensureTreatmentHistoryActions();
  } finally {
    enhancing = false;
  }
}
