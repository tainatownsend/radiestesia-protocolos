...
function addEvent(store, draft, input) {
  draft.events.push({ id:store.makeId('evt'), eventType:input.eventType, entityType:input.entityType, entityId:input.entityId,
...
export function latestTreatmentAssessment(state, treatmentId) {
  return [...(state.assessments || [])].filter((item)=>item.treatmentId===treatmentId)
...
export function canPlanFollowUpTreatment(state, treatmentId) {
  ...
  return !state.treatments.some((item)=>item.previousTreatmentId===treatmentId || item.recommendedByAssessmentId===assessment.id);
}
...
export function createFollowUpTreatment(store, treatmentId, input = {}) {
  ...
  if(!canPlanFollowUpTreatment(state,treatmentId))throw new Error('Esta avaliação final já originou um próximo ciclo.');
  ...
  const planned=createPlannedTreatment(store,{assistedEntityId:previous.assistedEntityId,title,notes:input.notes?.trim()||assessment.nextTreatmentWhen||null,components:input.components});
  ...
  store.setState((current)=>{
    ...
    addEvent(store,draft,{eventType:'FOLLOW_UP_TREATMENT_PLANNED',entityType:'Treatment',entityId:treatment.id,assistedEntityId:treatment.assistedEntityId,metadata:{title:treatment.title,previousTreatmentId:previous.id,recommendedByAssessmentId:assessment.id,plannedFor:treatment.plannedFor,componentCount:input.components.length}});
    return draft;
  });
