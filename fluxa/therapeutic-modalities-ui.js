...
function openConfig(){
  ...
  wrap.innerHTML=`<section class="sheet therapeutic-modalities-sheet"><div class="sheet-head"><div><p class="eyebrow">Preferências</p><h2>Terapias disponíveis</h2></div><button class="close-btn" type="button" data-close-modalities>×</button></div><p class="muted">Radiestesia é a base. Ative apenas as terapias complementares que você utiliza.</p><form id="therapeutic-modalities-form" class="form-grid"><div class="modality-base-card"><span class="eyebrow">Base do Fluxa</span><strong>Radiestesia</strong></div><fieldset class="field"><legend>Terapias complementares</legend><div class="checklist">${OPTIONAL.map(item=>`<label class="check-row"><input type="checkbox" name="enabledModality" value="${item.id}" ${settings.enabled.includes(item.id)?'checked':''}><span><strong>${item.label}</strong></span></label>`).join('')}</div></fieldset><div class="field"><label for="custom-modalities">Outras terapias <span class="muted">(opcional)</span></label><textarea id="custom-modalities" name="customModalities" placeholder="Uma terapia por linha">${esc(settings.custom.join('\n'))}</textarea><small class="muted">Inclua outras modalidades que façam parte da sua prática.</small></div><button class="btn primary wide" type="submit">Salvar terapias</button></form></section>`;
  document.body.appendChild(wrap);
}
function enhanceLibraryPreferences(){
  const main=document.querySelector('main');if(!main||main.querySelector(':scope > .eyebrow')?.textContent?.trim()!=='Biblioteca'||main.querySelector('[data-modality-settings-card]'))return;
...
function enhance(){if(enhancing)return;enhancing=true;try{enhanceLibraryPreferences();enhanceTreatmentForm();enhanceTreatmentCards();}finally{enhancing=false;}}
