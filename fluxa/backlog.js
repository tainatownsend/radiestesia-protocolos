...
export const BacklogEventType = Object.freeze({
  SESSION_CLOSE_CORRECTED: 'SESSION_CLOSE_CORRECTED',
  COMPONENT_ADDED: 'COMPONENT_ADDED',
  COMPONENT_STOPPED: 'COMPONENT_STOPPED',
  COMPONENT_REPLACED: 'COMPONENT_REPLACED',
  COMPONENT_RESCHEDULED: 'COMPONENT_RESCHEDULED',
  TREATMENT_FINAL_ASSESSMENT: 'TREATMENT_FINAL_ASSESSMENT'
});
...
export function correctForgottenSessionClose(store, sessionId, endedAt, confirmation = 'Encerramento corrigido posteriormente') {
  ...
  store.setState((current) => {
    ...
    const recordedAt = store.nowIso();
    target.status = 'CLOSED';
    target.endedAt = end.toISOString();
    target.closedRecordedAt = recordedAt;
    target.updatedAt = recordedAt;
    const run = {
      id: store.makeId('close'), sessionId, status: 'COMPLETED', startedAt: recordedAt, completedAt: recordedAt,
      confirmationSnapshot: confirmation, correction: true, actualEndedAt: end.toISOString(), recordedAt
    };
...
export function addTreatmentComponent(store, input) {
  const state = store.getState();
  if (input.sessionId) requireOpenSession(state, input.sessionId);
  ...
  const component = {
    id: store.makeId('cmp'), treatmentId: treatment.id, type: input.type || 'TOOL', name: input.name?.trim() || 'Componente terapêutico',
    instructions: input.instructions?.trim() || null, status: TreatmentStatus.IN_PROGRESS, startedAt: input.startedAt || now,
    durationValue: Number(input.durationValue) || null, durationUnit: input.durationUnit || null,
    expectedEndAt: addDuration(input.startedAt || now, input.durationValue, input.durationUnit),
    completedAt: null, interruptedAt: null, stoppedAt: null, replacedByComponentId: null,
    createdAt: now, updatedAt: now
  };
  store.setState((current) => {
    const draft = structuredClone(current);
    draft.treatmentComponents.push(component);
    addEvent(store, draft, { eventType: BacklogEventType.COMPONENT_ADDED, entityType: 'TreatmentComponent', entityId: component.id,
      sessionId: input.sessionId || null, assistedEntityId: treatment.assistedEntityId,
...
export function stopTreatmentComponent(store, componentId, input = {}) {
  store.setState((state) => {
    ...
    if (!component || ![TreatmentStatus.IN_PROGRESS, TreatmentStatus.INTERRUPTED].includes(component.status)) return draft;
    const treatment = draft.treatments.find((item) => item.id === component.treatmentId);
    const now = store.nowIso();
    component.status = 'STOPPED';
    component.stoppedAt = now;
    component.updatedAt = now;
    addEvent(store, draft, { eventType: BacklogEventType.COMPONENT_STOPPED, entityType: 'TreatmentComponent', entityId: component.id,
      sessionId: input.sessionId || null, assistedEntityId: treatment?.assistedEntityId || null,
      metadata: { treatmentId: component.treatmentId, name: component.name, reason: input.reason?.trim() || null } });
    return draft;
  });
}
...
export function replaceTreatmentComponent(store, componentId, input) {
  const state = store.getState();
  const old = state.treatmentComponents.find((item) => item.id === componentId);
  ...
  store.setState((current) => {
    const draft = structuredClone(current);
    const original = draft.treatmentComponents.find((item) => item.id === componentId);
    const treatment = draft.treatments.find((item) => item.id === original.treatmentId);
    const now = store.nowIso();
    original.status = 'REPLACED';
    original.stoppedAt = now;
    original.replacedByComponentId = replacement.id;
    original.updatedAt = now;
    addEvent(store, draft, { eventType: BacklogEventType.COMPONENT_REPLACED, entityType: 'TreatmentComponent', entityId: original.id,
      sessionId: input.sessionId || null, assistedEntityId: treatment?.assistedEntityId || null,
      metadata: { treatmentId: original.treatmentId, replacementComponentId: replacement.id, originalName: original.name, replacementName: replacement.name } });
    return draft;
  });
  return replacement;
}
  ...
  store.setState((current) => {
    ...
    draft.treatmentComponents.filter((item) => item.treatmentId === treatmentId && item.status === TreatmentStatus.INTERRUPTED).forEach((item) => {
      ...
      if (input.preserveRemainingDuration !== false && item.expectedEndAt && pauseMs) {
        ...
        addEvent(store, draft, { eventType: BacklogEventType.COMPONENT_RESCHEDULED, entityType: 'TreatmentComponent', entityId: item.id,
          assistedEntityId: target.assistedEntityId, metadata: { treatmentId, expectedEndAt: item.expectedEndAt, pauseMs } });
      }
    });
    addEvent(store, draft, { eventType: EventType.TREATMENT_RESUMED, entityType: 'Treatment', entityId: target.id,
      assistedEntityId: target.assistedEntityId, metadata: { preserveRemainingDuration: input.preserveRemainingDuration !== false, pauseMs } });
    return draft;
  });
}
...
export function recordStructuredFinalAssessment(store, input) {
  ...
  const treatment = state.treatments.find((item) => item.id === input.treatmentId);
  if (!treatment) throw new Error('Tratamento não encontrado.');
  const frequency = input.frequency?.trim() || null;
  const imbalancePercent = input.imbalancePercent === '' || input.imbalancePercent == null ? null : Number(input.imbalancePercent);
  if (imbalancePercent != null && (imbalancePercent < 0 || imbalancePercent > 100)) throw new Error('Desequilíbrio deve estar entre 0% e 100%.');
  const needsNewTreatment = Boolean(input.needsNewTreatment);
  const assessment = {
    id: store.makeId('assess'), treatmentId: treatment.id, sessionId: input.sessionId, assistedEntityId: treatment.assistedEntityId,
    frequency, imbalancePercent, needsNewTreatment, nextTreatmentWhen: input.nextTreatmentWhen?.trim() || null,
    notes: input.notes?.trim() || null, occurredAt: store.nowIso(), createdAt: store.nowIso()
  };
  store.setState((current) => {
    const draft = structuredClone(current);
    if (!Array.isArray(draft.assessments)) draft.assessments = [];
    draft.assessments.push(assessment);
    addEvent(store, draft, { eventType: BacklogEventType.TREATMENT_FINAL_ASSESSMENT, entityType: 'Assessment', entityId: assessment.id,
      sessionId: input.sessionId, assistedEntityId: treatment.assistedEntityId,
      metadata: { treatmentId: treatment.id, frequency, imbalancePercent, needsNewTreatment, nextTreatmentWhen: assessment.nextTreatmentWhen } });
    return draft;
  });
  return assessment;
}
...
export function validateAssistedInput(input) {
  ...
  if (type === 'GROUP') {
    ...
    if (members.some((m) => !m.fullName?.trim() || !m.birthDate)) throw new Error('Cada integrante do grupo precisa de nome completo e data de nascimento.');
  }
  return true;
}
