...
function assistedIds(state,sessionId){const ids=new Set();state.events.filter((e)=>e.sessionId===sessionId&&e.assistedEntityId).forEach((e)=>ids.add(e.assistedEntityId));sessionInvestigations(state,sessionId).forEach((i)=>i.assistedEntityId&&ids.add(i.assistedEntityId));state.reikiApplications.filter((r)=>r.sessionId===sessionId).forEach((r)=>r.assistedEntityId&&ids.add(r.assistedEntityId));return [...ids];}
function dataFor(state,sessionId,assistedId){
  ...
  const invIds=new Set(investigations.map((i)=>i.id));
  return {
    investigations,
    findings:state.findings.filter((f)=>invIds.has(f.investigationId)),
...
function internalReportBody(state,sessionId){
  const ids=assistedIds(state,sessionId);
...
// Window capture runs before the older document-level handler and becomes the canonical report action.
