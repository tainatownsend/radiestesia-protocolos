...
export function buildGraphApplication(input={},tools=[],startedAt=new Date().toISOString()){
  ...
  return {
    id:input.id||id('graphapp'),
    graphName,
    toolId:match?.id||null,
    toolSnapshot:match?{id:match.id,type:match.type,name:match.name}:null,
    manual:!match,
    durationValue,
    durationUnit,
    noDuration,
...
export function buildTreatmentItem(input={},tools=[],startedAt=new Date().toISOString()){
  ...
  return {semanticsVersion:2,itemLabel,commands};
}
...
export function enrichComponentWithTreatmentItem(store,componentId,input){
  ...
  store.setState(state=>{
    ...
    const startedAt=component.startedAt||store.nowIso();
    const item=buildTreatmentItem(input,draft.tools||[],startedAt);
    component.semanticsVersion=2;
    component.itemLabel=item.itemLabel;
    component.name=item.itemLabel;
    component.commands=item.commands;
    component.instructions=item.commands.map(command=>command.text).join('\n');
    component.expectedEndAt=latestExpectedEndAt(item);
    component.durationValue=null;
    component.durationUnit=null;
    component.updatedAt=store.nowIso();
    enriched=structuredClone(component);
    return draft;
  });
  return enriched;
}
...
export function treatmentItemView(component){
  if(component?.semanticsVersion===2&&Array.isArray(component.commands))return {itemLabel:component.itemLabel||component.name,commands:component.commands,legacy:false};
  const graphName=text(component?.toolSnapshot?.name||component?.name||'Gráfico');
  const commandText=text(component?.instructions||'Aplicação registrada');
  return {itemLabel:component?.name||graphName,legacy:true,commands:[{id:`legacy_${component?.id||'item'}`,text:commandText,graphApplications:[{id:`legacy_graph_${component?.id||'item'}`,graphName,toolId:component?.toolId||null,toolSnapshot:component?.toolSnapshot||null,manual:!component?.toolId,durationValue:component?.durationValue??null,durationUnit:component?.durationUnit||'DAY',noDuration:!component?.expectedEndAt,startedAt:component?.startedAt||null,expectedEndAt:component?.expectedEndAt||null}]}]};
}
