...
function injectCancel() {
  const timer = document.querySelector('.timer-sheet');
  if (!timer || timer.querySelector('[data-cancel-reiki]')) return;
  const appId = timer.querySelector('[data-live-timer]')?.dataset.liveTimer;
...
document.addEventListener('click', (event) => {
  const start = event.target.closest('[data-action="reiki"],[data-start-outside-reiki]');
  if (!start) return;
...
document.addEventListener('click', (event) => {
  ...
  try {
    ...
    document.querySelector('[data-action="dismiss-sheet"], [data-close-outside-reiki]')?.click();
    location.reload();
