...
export const EventType = Object.freeze({
  ...
  TREATMENT_INTERRUPTED: 'TREATMENT_INTERRUPTED',
  TREATMENT_RESUMED: 'TREATMENT_RESUMED',
  TREATMENT_REVIEWED: 'TREATMENT_REVIEWED',
  TREATMENT_COMPLETED: 'TREATMENT_COMPLETED',
  COMPONENT_STARTED: 'COMPONENT_STARTED',
  COMPONENT_COMPLETED: 'COMPONENT_COMPLETED',
  REIKI_STARTED: 'REIKI_STARTED',
  REIKI_PAUSED: 'REIKI_PAUSED',
  REIKI_RESUMED: 'REIKI_RESUMED',
  REIKI_COMPLETED: 'REIKI_COMPLETED',
...
export function startSession(store) {
  const existing = getOpenSession(store.getState());
  if (existing) return existing;
  const session = {
    id: store.makeId('ses'), status: SessionStatus.OPEN, startedAt: store.nowIso(), endedAt: null,
    closedRecordedAt: null, currentAssistedEntityId: null, createdAt: store.nowIso(), updatedAt: store.nowIso()
  };
  store.setState((state) => {
    const draft = structuredClone(state);
    draft.sessions.push(session);
    addEvent(store, draft, { eventType: EventType.SESSION_STARTED, entityType: 'Session', entityId: session.id, sessionId: session.id });
    return draft;
  });
  return session;
}
...
export function closeSession(store, sessionId, input = {}) {
  ...
  store.setState((current) => {
    ...
    target.closedRecordedAt = store.nowIso();
    target.updatedAt = store.nowIso();
    const run = {
      id: store.makeId('close'), sessionId, status: 'COMPLETED', startedAt: store.nowIso(), completedAt: store.nowIso(),
      confirmationSnapshot: input.confirmation || 'Procedimento de encerramento concluído'
    };
    draft.closingRuns.push(run);
    addEvent(store, draft, { eventType: EventType.CLOSING_COMPLETED, entityType: 'ClosingRun', entityId: run.id, sessionId });
    addEvent(store, draft, { eventType: EventType.SESSION_CLOSED, entityType: 'Session', entityId: session.id, sessionId, occurredAt: endedAt });
    return draft;
  });
}
...
export function startPreparation(store, sessionId) {
  ...
  const run = {
    id: store.makeId('prep'), sessionId, status: 'IN_PROGRESS', startedAt: store.nowIso(), completedAt: null,
    steps: PREPARATION_STEPS.map((step) => ({ ...step, completed: false, completedAt: null }))
  };
  store.setState((state) => {
    const draft = structuredClone(state);
    draft.preparationRuns.push(run);
    addEvent(store, draft, { eventType: EventType.PREPARATION_STARTED, entityType: 'PreparationRun', entityId: run.id, sessionId });
    return draft;
  });
  return run;
}
...
export function latestPreparation(state, sessionId) {
  return [...state.preparationRuns].filter((run) => run.sessionId === sessionId).sort((a, b) => b.startedAt.localeCompare(a.startedAt))[0] || null;
}
...
export function completePreparation(store, runId) {
  store.setState((state) => {
    ...
    addEvent(store, draft, { eventType: EventType.PREPARATION_COMPLETED, entityType: 'PreparationRun', entityId: run.id, sessionId: run.sessionId });
    return draft;
  });
}
...
export function createAssistedEntity(store, input) {
  ...
  store.setState((state) => {
    ...
    addEvent(store, draft, { eventType: EventType.ASSISTED_CREATED, entityType: 'AssistedEntity', entityId: entity.id, assistedEntityId: entity.id });
    return draft;
  });
  return entity;
}
...
export function selectAssistedForSession(store, sessionId, assistedEntityId) {
  store.setState((state) => {
    ...
    const assisted = getAssisted(draft, assistedEntityId);
    ...
    addEvent(store, draft, { eventType: EventType.SESSION_ASSISTED_SELECTED, entityType: 'Session', entityId: session.id, sessionId, assistedEntityId });
    return draft;
  });
}
...
export function startInvestigation(store, sessionId, assistedEntityId) {
  ...
  const investigation = {
    id: store.makeId('inv'), originSessionId: sessionId, currentSessionId: sessionId, assistedEntityId,
    protocolId: MVP_PROTOCOL.id, protocolVersionId: MVP_PROTOCOL.versionId, protocolSnapshot: structuredClone(MVP_PROTOCOL),
    status: 'IN_PROGRESS', currentIndex: 0, answers: [], startedAt: store.nowIso(), completedAt: null, updatedAt: store.nowIso()
  };
  store.setState((current) => {
    const draft = structuredClone(current);
    draft.investigations.push(investigation);
    addEvent(store, draft, { eventType: EventType.INVESTIGATION_STARTED, entityType: 'Investigation', entityId: investigation.id, sessionId, assistedEntityId,
...
export function resumeInvestigation(store, investigationId, sessionId) {
  ...
  store.setState((current) => {
    ...
    if (target.currentSessionId !== sessionId) {
      ...
      target.updatedAt = store.nowIso();
      addEvent(store, draft, { eventType: EventType.INVESTIGATION_RESUMED, entityType: 'Investigation', entityId: target.id, sessionId,
        assistedEntityId: target.assistedEntityId, metadata: { originSessionId: target.originSessionId } });
    }
    const session = draft.sessions.find((item) => item.id === sessionId);
  ...
  store.setState((source) => {
    ...
    if (investigation.currentIndex < investigation.protocolSnapshot.questions.length - 1) {
      ...
      addEvent(store, draft, { eventType: EventType.INVESTIGATION_COMPLETED, entityType: 'Investigation', entityId: investigation.id,
        sessionId: investigation.currentSessionId, assistedEntityId: investigation.assistedEntityId, metadata: { protocolName: investigation.protocolSnapshot.name } });
    }
...
export function confirmFindings(store, investigationId, questionIds) {
  ...
  store.setState((source) => {
    ...
    for (const questionId of questionIds) {
      ...
      const finding = {
        id: store.makeId('find'), assistedEntityId: investigation.assistedEntityId, investigationId, sourceQuestionId: questionId,
        classification: 'FACTOR_RELEVANT', title: answer.questionTextSnapshot, status: 'IDENTIFIED', createdAt: store.nowIso()
      };
      ...
      addEvent(store, draft, { eventType: EventType.FINDING_IDENTIFIED, entityType: 'Finding', entityId: finding.id,
        sessionId: investigation.currentSessionId, assistedEntityId: investigation.assistedEntityId, metadata: { title: finding.title } });
    }
    return draft;
  });
  return created;
...
export function createTreatment(store, input) {
  ...
  const treatment = {
    id: store.makeId('trt'), assistedEntityId: input.assistedEntityId, originSessionId: input.sessionId,
    findingIds: [...new Set(input.findingIds || [])], title: input.title?.trim() || 'Tratamento', status: TreatmentStatus.IN_PROGRESS,
    startedAt, completedAt: null, interruptedAt: null, resumedAt: null, createdAt: store.nowIso(), updatedAt: store.nowIso()
  };
  const component = {
    id: store.makeId('cmp'), treatmentId: treatment.id, type: 'TOOL', name: input.componentName?.trim() || 'Componente terapêutico',
    instructions: input.instructions?.trim() || null, status: TreatmentStatus.IN_PROGRESS, startedAt,
    durationValue: Number(input.durationValue) || null, durationUnit: input.durationUnit || null,
    expectedEndAt: addDuration(startedAt, input.durationValue, input.durationUnit), completedAt: null, interruptedAt: null,
  ...
  store.setState((current) => {
    ...
    addEvent(store, draft, { eventType: EventType.TREATMENT_CREATED, entityType: 'Treatment', entityId: treatment.id,
      sessionId: input.sessionId, assistedEntityId: input.assistedEntityId, metadata: { title: treatment.title } });
    addEvent(store, draft, { eventType: EventType.TREATMENT_STARTED, entityType: 'Treatment', entityId: treatment.id,
      sessionId: input.sessionId, assistedEntityId: input.assistedEntityId, metadata: { title: treatment.title } });
    addEvent(store, draft, { eventType: EventType.COMPONENT_STARTED, entityType: 'TreatmentComponent', entityId: component.id,
...
export function interruptTreatment(store, treatmentId, reason = '') {
  store.setState((state) => {
    ...
    if (!treatment || treatment.status !== TreatmentStatus.IN_PROGRESS) return draft;
    treatment.status = TreatmentStatus.INTERRUPTED;
    treatment.interruptedAt = store.nowIso();
    treatment.updatedAt = store.nowIso();
    draft.treatmentComponents.filter((item) => item.treatmentId === treatmentId && item.status === TreatmentStatus.IN_PROGRESS)
      .forEach((item) => { item.status = TreatmentStatus.INTERRUPTED; item.interruptedAt = store.nowIso(); item.updatedAt = store.nowIso(); });
    addEvent(store, draft, { eventType: EventType.TREATMENT_INTERRUPTED, entityType: 'Treatment', entityId: treatment.id,
      assistedEntityId: treatment.assistedEntityId, metadata: { reason: reason.trim() || null } });
    return draft;
  });
}
...
export function resumeTreatment(store, treatmentId) {
  store.setState((state) => {
    const draft = structuredClone(state);
    const treatment = draft.treatments.find((item) => item.id === treatmentId);
    if (!treatment || treatment.status !== TreatmentStatus.INTERRUPTED) return draft;
    treatment.status = TreatmentStatus.IN_PROGRESS;
    treatment.resumedAt = store.nowIso();
    treatment.updatedAt = store.nowIso();
    draft.treatmentComponents.filter((item) => item.treatmentId === treatmentId && item.status === TreatmentStatus.INTERRUPTED)
      .forEach((item) => { item.status = TreatmentStatus.IN_PROGRESS; item.updatedAt = store.nowIso(); });
    addEvent(store, draft, { eventType: EventType.TREATMENT_RESUMED, entityType: 'Treatment', entityId: treatment.id,
...
export function reviewTreatment(store, input) {
  ...
  const review = {
    id: store.makeId('rev'), treatmentId: treatment.id, sessionId: input.sessionId, assistedEntityId: treatment.assistedEntityId,
    verifiedComplete: Boolean(input.verifiedComplete), imbalancePercent: input.imbalancePercent === '' || input.imbalancePercent == null ? null : Number(input.imbalancePercent),
    notes: input.notes?.trim() || null, reviewedAt: store.nowIso(), createdAt: store.nowIso()
  };
  store.setState((current) => {
    const draft = structuredClone(current);
    const target = draft.treatments.find((item) => item.id === treatment.id);
    draft.treatmentReviews.push(review);
    addEvent(store, draft, { eventType: EventType.TREATMENT_REVIEWED, entityType: 'TreatmentReview', entityId: review.id,
      sessionId: input.sessionId, assistedEntityId: treatment.assistedEntityId,
      metadata: { treatmentId: treatment.id, verifiedComplete: review.verifiedComplete, imbalancePercent: review.imbalancePercent } });
    if (review.verifiedComplete) {
      const now = store.nowIso();
      draft.treatmentComponents.filter((item) => item.treatmentId === treatment.id && item.status !== TreatmentStatus.COMPLETED)
        .forEach((item) => {
          item.status = TreatmentStatus.COMPLETED; item.completedAt = now; item.updatedAt = now;
          addEvent(store, draft, { eventType: EventType.COMPONENT_COMPLETED, entityType: 'TreatmentComponent', entityId: item.id,
            sessionId: input.sessionId, assistedEntityId: treatment.assistedEntityId, metadata: { treatmentId: treatment.id, name: item.name } });
        });
      target.status = TreatmentStatus.COMPLETED;
      target.completedAt = now;
      target.updatedAt = now;
      addEvent(store, draft, { eventType: EventType.TREATMENT_COMPLETED, entityType: 'Treatment', entityId: target.id,
        sessionId: input.sessionId, assistedEntityId: target.assistedEntityId, metadata: { imbalancePercent: review.imbalancePercent } });
    }
    return draft;
  });
  return review;
...
export function treatmentNeedsReview(state, treatment) {
  if (!treatment || treatment.status !== TreatmentStatus.IN_PROGRESS) return false;
  return state.treatmentComponents.some((item) => item.treatmentId === treatment.id && item.status === TreatmentStatus.IN_PROGRESS && item.expectedEndAt && new Date(item.expectedEndAt).getTime() <= Date.now());
}

function activeElapsedMs(application, now = Date.now()) {
  return (application.intervals || []).reduce((total, interval) => {
...
export function reikiElapsedSeconds(application, now = Date.now()) {
  return Math.floor(activeElapsedMs(application, now) / 1000);
}

export function startReiki(store, sessionId, assistedEntityId) {
  const state = store.getState();
  requireOpenSession(state, sessionId);
  const existing = state.reikiApplications.find((item) => item.sessionId === sessionId && item.status === 'RUNNING');
  if (existing) return existing;
  const now = store.nowIso();
  const application = {
    id: store.makeId('reiki'), sessionId, assistedEntityId, status: 'RUNNING', startedAt: now, endedAt: null,
    durationSeconds: null, notes: null, intervals: [{ id: store.makeId('int'), startedAt: now, endedAt: null }], createdAt: now, updatedAt: now
  };
  store.setState((current) => {
    const draft = structuredClone(current);
    draft.reikiApplications.push(application);
    addEvent(store, draft, { eventType: EventType.REIKI_STARTED, entityType: 'ReikiApplication', entityId: application.id,
      sessionId, assistedEntityId });
    return draft;
  });
  return application;
}
...
export function pauseReiki(store, applicationId) {
  store.setState((state) => {
    ...
    app.status = 'PAUSED'; app.updatedAt = store.nowIso();
    addEvent(store, draft, { eventType: EventType.REIKI_PAUSED, entityType: 'ReikiApplication', entityId: app.id,
...
export function resumeReiki(store, applicationId) {
  store.setState((state) => {
    const draft = structuredClone(state);
    const app = draft.reikiApplications.find((item) => item.id === applicationId && item.status === 'PAUSED');
    if (!app) return draft;
    requireOpenSession(draft, app.sessionId);
    app.intervals.push({ id: store.makeId('int'), startedAt: store.nowIso(), endedAt: null });
    app.status = 'RUNNING'; app.updatedAt = store.nowIso();
    addEvent(store, draft, { eventType: EventType.REIKI_RESUMED, entityType: 'ReikiApplication', entityId: app.id,
      sessionId: app.sessionId, assistedEntityId: app.assistedEntityId });
    return draft;
  });
}
...
export function completeReiki(store, applicationId, notes = '') {
  store.setState((state) => {
    ...
    app.status = 'COMPLETED'; app.endedAt = store.nowIso(); app.notes = notes.trim() || null;
    app.durationSeconds = reikiElapsedSeconds(app, new Date(app.endedAt).getTime()); app.updatedAt = store.nowIso();
    addEvent(store, draft, { eventType: EventType.REIKI_COMPLETED, entityType: 'ReikiApplication', entityId: app.id,
      sessionId: app.sessionId, assistedEntityId: app.assistedEntityId, metadata: { durationSeconds: app.durationSeconds } });
    return draft;
  });
}

export function recordReikiRetrospective(store, input) {
  const seconds = Math.max(0, Number(input.durationMinutes || 0) * 60);
  const occurredAt = input.occurredAt || store.nowIso();
  const ended = new Date(occurredAt);
  const started = new Date(ended.getTime() - seconds * 1000);
  const application = {
    id: store.makeId('reiki'), sessionId: null, assistedEntityId: input.assistedEntityId, status: 'COMPLETED',
    startedAt: started.toISOString(), endedAt: ended.toISOString(), durationSeconds: seconds, notes: input.notes?.trim() || null,
    intervals: [{ id: store.makeId('int'), startedAt: started.toISOString(), endedAt: ended.toISOString() }], createdAt: store.nowIso(), updatedAt: store.nowIso()
  };
  store.setState((state) => {
    const draft = structuredClone(state);
    draft.reikiApplications.push(application);
    addEvent(store, draft, { eventType: EventType.REIKI_COMPLETED, entityType: 'ReikiApplication', entityId: application.id,
      assistedEntityId: input.assistedEntityId, occurredAt: application.endedAt,
      metadata: { durationSeconds: application.durationSeconds, retrospective: true } });
    return draft;
  });
  return application;
}
...
export function addSessionNote(store, sessionId, assistedEntityId, body) {
  ...
  store.setState((state) => {
    const draft = structuredClone(state);
    addEvent(store, draft, { eventType: EventType.NOTE_CREATED, entityType: 'Note', entityId: store.makeId('note'),
      sessionId, assistedEntityId, metadata: { body: text } });
    return draft;
  });
}
