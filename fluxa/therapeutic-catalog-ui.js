...
function rootDialog(){
  if(!activeRootId)return;const state=store.getState(),inv=state.investigations.find(i=>i.id===activeRootId);if(!inv){closeRootDialog();return;}const assisted=state.assistedEntities.find(a=>a.id===inv.assistedEntityId),node=currentRootNode(inv);
  let wrap=document.querySelector('#root-protocol-overlay');if(!wrap){wrap=document.createElement('div');wrap.id='root-protocol-overlay';wrap.className='modal-backdrop protocol-stable-overlay';document.body.appendChild(wrap);}
...
document.addEventListener('click',e=>{
  ...
  if(b.dataset.rootAnswer){try{const peers=b.closest('.binary-actions')?.querySelectorAll('button');peers?.forEach(x=>x.disabled=true);answerRootProtocol(activeRootId,b.dataset.rootAnswer);rootDialog();}catch(error){alert(error.message);b.closest('.binary-actions')?.querySelectorAll('button').forEach(x=>x.disabled=false);}return;}
