...
function protocolCard(protocol,kind='root'){
  ...
  return `<article class="therapeutic-protocol-card" data-protocol-card data-protocol-search-text="${esc(search)}"><div><p class="eyebrow">${esc(protocol.category)}</p><h3>${esc(protocol.name)}</h3><p>${esc(protocol.description||'')}</p></div><button class="btn secondary" ${action} aria-label="${active?'Retomar':'Iniciar'} ${esc(protocol.name)}">${active?'Retomar':'Abrir'}</button></article>`;
}
function renderCatalog(sheet,rootProtocols){
  ...
  sheet.innerHTML=`<div class="sheet-head"><div><p class="eyebrow">Investigar</p><h2>O que você quer compreender agora?</h2></div><button class="close-btn" data-close-investigation-chooser>×</button></div>
    ...
    <div class="catalog-sections">${sections.map(([name,items],index)=>`<details class="catalog-group" data-catalog-group data-default-open="${index<2?'true':'false'}" ${index<2?'open':''}><summary class="section-head"><h3>${esc(name)}</h3><span class="muted">${items.length}</span></summary><div class="catalog-list">${items.map(p=>protocolCard(p,p._kind)).join('')}</div></details>`).join('')}</div>
...
function applyFilter(overlay){
  ...
  const filtering=Boolean(query.length||theme.length);
  overlay.querySelectorAll('[data-catalog-group]').forEach(group=>{const hasVisible=[...group.querySelectorAll('[data-protocol-card]')].some(c=>!c.hidden);group.hidden=!hasVisible;if(filtering)group.open=hasVisible;else group.open=group.dataset.defaultOpen==='true';});
