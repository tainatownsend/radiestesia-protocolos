...
function blankGraph() { return { graphName: '', durationValue: '', durationUnit: 'DAY' }; }
function blankCommand() { return { text: '', graphApplications: [blankGraph()] }; }
function blankItem(label = '') { return { itemLabel: label, commands: [blankCommand()] }; }
...
function currentWorkflowModel() { return liveMode ? deriveV2Model(store.getState()) : model; }
function requireTreatmentActionAllowed(action, treatmentId) {
  ...
  const isRecommendedReview = current.nextActionCode === 'TREATMENT_REVIEW' && action === 'review' && current.nextActionTreatmentId === treatmentId;
  const isRecommendedFinal = current.nextActionCode === 'TREATMENT_FINAL' && action === 'final' && current.nextActionTreatmentId === treatmentId;
...
function requireNewTreatmentAllowed() {
  const current = currentWorkflowModel();
  if (isContinuityLocked(current.nextActionCode)) throw new Error('Conclua a próxima ação recomendada em Hoje antes de criar outro tratamento.');
}
function assistedContextChangeAllowed() { return !isContinuityLocked(currentWorkflowModel().nextActionCode); }

const ui = {
  route: 'today', sheet: liveMode ? null : (model.overlay === 'preparation' ? 'preparation' : null), assistedCreate: false,
  error: '', activeTreatmentId: null, reviewComponentId: null, treatmentDraft: blankTreatmentDraft(), justClosedSessionId: null,
...
function clearInlineError() { ui.error = ''; root.querySelector('.v2-inline-error')?.remove(); }
function showInlineError(error) {
  ...
  alert.className = 'v2-inline-error'; alert.setAttribute('role', 'alert'); alert.textContent = message; body.prepend(alert);
}
function closeSheet() {
  const closingSheet = ui.sheet;
  ui.sheet = null; ui.assistedCreate = false; ui.reviewComponentId = null;
  if (closingSheet === 'settings') ui.importPreview = null;
  ui.error = ''; render(); queueMicrotask(() => opener?.focus?.());
}
function openSheet(name, source = null) {
  if (source) opener = source;
  ui.sheet = name; ui.error = ''; scheduleRender({ focusDialog: true });
}
...
function syncTreatmentDraftFromDom() {
  ...
  root.querySelectorAll('[data-v2-treatment-draft]').forEach((field) => {
    ...
    const itemIndex = Number(field.dataset.itemIndex), commandIndex = Number(field.dataset.commandIndex), graphIndex = Number(field.dataset.graphIndex);
    if (kind === 'title' || kind === 'objective') { ui.treatmentDraft[kind] = field.value; return; }
    const item = ui.treatmentDraft.items?.[itemIndex]; if (!item) return;
    if (kind === 'itemLabel') item.itemLabel = field.value;
    const command = item.commands?.[commandIndex]; if (!command) return;
...
function treatmentInput() {
  ...
  const modalities = (model.modalityOptions || []).filter((item) => ui.treatmentDraft.modalities.includes(item.id)).map((item) => ({ id:item.id, label:item.label }));
  return { ...structuredClone(ui.treatmentDraft), modalities };
}
function treatmentById(id) { return (model.treatments || []).find((item) => item.id === id) || null; }
...
function performTreatmentAction(action, treatmentId, source = null) {
  if (!liveMode) return;
  if (source) opener = source;
  ui.activeTreatmentId = treatmentId; ui.error = '';
  try {
    requireTreatmentActionAllowed(action, treatmentId);
    if (action === 'start') { startPlannedTreatmentV2(store, treatmentId); ui.sheet = 'treatment-workspace'; }
    else if (action === 'resume') { resumeTreatmentV2(store, treatmentId); ui.sheet = 'treatment-workspace'; }
      ...
      ui.reviewComponentId = due.id; ui.sheet = 'treatment-review';
    } else if (action === 'final') ui.sheet = 'final-assessment';
    else ui.sheet = 'treatment-workspace';
...
function performPrimaryAction(source = null) {
  ...
  try {
    ...
    if (action === 'START_SESSION') { beginSession(store); ui.sheet = 'preparation'; ui.justClosedSessionId = null; }
    else if (action === 'PREPARE') { prepareCurrentSession(store); ui.sheet = 'preparation'; }
    else if (action === 'SELECT_ASSISTED') ui.sheet = 'assisted';
    else if (action === 'HAWKINS') ui.sheet = 'hawkins';
    else if (action === 'INVESTIGATE') { beginTriage(store); ui.sheet = 'triage'; }
    else if (action === 'REVIEW_FINDINGS') ui.sheet = 'findings';
    else if (action === 'TREATMENT_REVIEW' || action === 'TREATMENT_FINAL') {
      performTreatmentAction(action === 'TREATMENT_REVIEW' ? 'review' : 'final', model.nextActionTreatmentId, source); return;
    } else if (action === 'TREATMENT_RESUME') { performTreatmentAction('resume', model.nextActionTreatmentId, source); return; }
    else if (action === 'REIKI') ui.sheet = 'reiki';
    else if (action === 'CLOSE_SESSION') ui.sheet = 'closing';
    if (source) opener = source;
  ...
  try {
    if (ui.sheet === 'preparation') {
      advancePreparation(store, {
        frequencyValue: root.querySelector('[data-v2-frequency]')?.value || '',
        protectionNotes: root.querySelector('[data-v2-protection-notes]')?.value || '',
        permissionNotes: root.querySelector('[data-v2-permission-notes]')?.value || '',
      });
      ...
      const closedId = closeCurrentSessionV2(store, { confirmation: root.querySelector('[data-v2-closing-confirmation]')?.value || '' });
      ui.justClosedSessionId = closedId; ui.route = 'today';
...
root.addEventListener('click', (event) => {
  ...
  if (!target) return;
  if (target.matches('[data-v2-close-sheet]')) { closeSheet(); return; }
  if (target.matches('[data-v2-primary]')) { submitCurrentSheet(); return; }
  if (target.matches('[data-v2-secondary]')) {
    if (ui.sheet === 'assisted' && ui.assistedCreate) { ui.assistedCreate = false; scheduleRender({ focusDialog:true }); }
    else closeSheet();
    return;
  }
  if (target.matches('[data-v2-route]')) {
    ui.route = target.dataset.v2Route; ui.sheet = null; ui.error = ''; ui.historySessionId = null;
    if (ui.route !== 'library') ui.librarySection = 'home';
    render(); return;
  }
  if (target.matches('[data-v2-open-settings]')) { openSheet('settings', target); return; }
  ...
  if (target.matches('[data-v2-triage-back]')) { if (liveMode) { stepBackTriage(store, model.investigation?.id); scheduleRender({ focusDialog:true }); } return; }
  if (target.matches('[data-v2-treatment-submit]')) {
    ...
    try {
      ...
      ui.activeTreatmentId = treatment.id; ui.sheet = start ? 'treatment-workspace' : null; ui.route = 'treatments'; ui.error = ''; render();
    } catch (error) { showInlineError(error); }
    return;
  }
  if (target.matches('[data-v2-treatment-action]')) { performTreatmentAction(target.dataset.v2TreatmentAction, target.dataset.treatmentId, target); return; }
  if (target.matches('[data-v2-review-component]')) {
    ...
    try {
      ...
      ui.activeTreatmentId = treatmentId; ui.reviewComponentId = componentId; openSheet('treatment-review', target);
    } catch (error) { showInlineError(error); }
    return;
  }
  if (target.matches('[data-v2-review-save]')) {
    ...
    try {
      ...
      ui.sheet = 'treatment-workspace'; ui.reviewComponentId = null; scheduleRender({ focusDialog:true });
    } catch (error) { showInlineError(error); }
    return;
  }
  ...
  if (target.matches('[data-v2-closing-reiki]')) { openSheet('reiki', target); return; }
  if (target.matches('[data-v2-library-section]')) { ui.librarySection = target.dataset.v2LibrarySection; ui.route = 'library'; render(); return; }
  if (target.matches('[data-v2-library-back]')) { ui.librarySection = 'home'; render(); return; }
  if (target.matches('[data-v2-library-new-assisted]')) { openSheet('library-new-assisted', target); return; }
  if (target.matches('[data-v2-history-session]')) { ui.route = 'history'; ui.historySessionId = target.dataset.v2HistorySession; ui.justClosedSessionId = null; render(); return; }
  if (target.matches('[data-v2-history-back]')) { ui.historySessionId = null; render(); return; }
  if (target.matches('[data-v2-open-history]')) { ui.route = 'history'; ui.justClosedSessionId = null; render(); return; }
  if (target.matches('[data-v2-add-item]')) { syncTreatmentDraftFromDom(); ui.treatmentDraft.items.push(blankItem()); renderPreservingSheetScroll(); return; }
  if (target.matches('[data-v2-remove-item]')) { syncTreatmentDraftFromDom(); ui.treatmentDraft.items.splice(Number(target.dataset.itemIndex),1); renderPreservingSheetScroll(); return; }
  if (target.matches('[data-v2-add-command]')) { syncTreatmentDraftFromDom(); ui.treatmentDraft.items[Number(target.dataset.itemIndex)]?.commands.push(blankCommand()); renderPreservingSheetScroll(); return; }
  if (target.matches('[data-v2-remove-command]')) { syncTreatmentDraftFromDom(); ui.treatmentDraft.items[Number(target.dataset.itemIndex)]?.commands.splice(Number(target.dataset.commandIndex),1); renderPreservingSheetScroll(); return; }
  if (target.matches('[data-v2-add-graph]')) { syncTreatmentDraftFromDom(); ui.treatmentDraft.items[Number(target.dataset.itemIndex)]?.commands[Number(target.dataset.commandIndex)]?.graphApplications.push(blankGraph()); renderPreservingSheetScroll(); return; }
  if (target.matches('[data-v2-remove-graph]')) { syncTreatmentDraftFromDom(); ui.treatmentDraft.items[Number(target.dataset.itemIndex)]?.commands[Number(target.dataset.commandIndex)]?.graphApplications.splice(Number(target.dataset.graphIndex),1); renderPreservingSheetScroll(); return; }
  if (target.matches('[data-v2-settings-save-modalities]')) {
    ...
    try {
      ...
      const custom = String(root.querySelector('[data-v2-settings-custom-modalities]')?.value || '').split(/\r?\n/).map((value) => value.trim()).filter(Boolean);
      store.setState((state) => {
        const draft = structuredClone(state); draft.settings = draft.settings || {}; draft.settings.therapeuticModalities = { enabled, custom }; return draft;
      });
      renderPreservingSheetScroll();
    } catch (error) { showInlineError(error); }
    return;
  }
  if (target.matches('[data-v2-settings-export]')) { if (liveMode) { try { exportLocalDataFile(); model = deriveLiveModel(); scheduleRender({ focusDialog:true }); } catch (error) { showInlineError(error); } } return; }
  if (target.matches('[data-v2-settings-import-apply]')) {
    if (!liveMode || !ui.importPreview?.normalized) return;
    try { requireDataReplacementIdle(); const normalized = structuredClone(ui.importPreview.normalized); store.setState(() => normalized); ui.importPreview = null; renderPreservingSheetScroll(); }
    catch (error) { showInlineError(error); }
    return;
  }
  if (target.matches('[data-v2-settings-recover]')) {
    if (!liveMode) return;
    try { requireDataReplacementIdle(); recoverLocalData(); loadState({ force:true }); ui.sheet = null; render(); }
    catch (error) { showInlineError(error); }
    return;
  }

  const action = target.closest('[data-v2-preview-action]');
  if (!action) return;
  opener = action;
  const name = action.dataset.v2PreviewAction;
  if (name === 'next' || name === 'start-session') { performPrimaryAction(action); return; }
  if (name === 'change-assisted') {
    if (liveMode && assistedContextChangeAllowed()) openSheet('assisted', action);
    return;
  }
  if (name === 'investigate') { if (liveMode) { try { beginTriage(store); openSheet('triage', action); } catch (error) { showInlineError(error); } } return; }
  if (name === 'treat') { if (liveMode) { try { requireNewTreatmentAllowed(); openTreatmentComposer(action); } catch (error) { showInlineError(error); } } return; }
  if (name === 'reiki') { openSheet('reiki', action); return; }
  if (name === 'close-session') openSheet('closing', action);
});
...
root.addEventListener('change', async (event) => {
  ...
  if (target.matches('[data-v2-settings-import-file]')) {
    ...
    try {
      ...
      let parsed; try { parsed = JSON.parse(text); } catch (_) { throw new Error('Este arquivo não contém um backup JSON válido do Fluxa.'); }
      const normalized = validateImportPayload(parsed);
      ui.importPreview = {
        name:file.name, normalized,
        summary:{ sessions:normalized.sessions?.length || 0, assisteds:normalized.assistedEntities?.length || 0, treatments:normalized.treatments?.length || 0, resources:normalized.tools?.length || 0 },
      };
      renderPreservingSheetScroll();
    } catch (error) { ui.importPreview = null; showInlineError(error); }
...
root.addEventListener('input', (event) => {
  ...
  if (target.matches('[data-v2-assisted-search-input]')) {
    ...
    root.querySelectorAll('[data-v2-assisted-search]').forEach((row) => { row.hidden = needle && !String(row.dataset.v2AssistedSearch || '').includes(needle); });
  }
  if (target.matches('[data-v2-library-search]')) {
    const needle = normalizeSearch(target.value);
    root.querySelectorAll('[data-v2-library-search-text]').forEach((row) => { row.hidden = needle && !normalizeSearch(row.dataset.v2LibrarySearchText || '').includes(needle); });
  }
});
