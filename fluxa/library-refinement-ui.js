...
function decorateCards(){const state=store.getState();document.querySelectorAll('[data-basic-tool-library] [data-library-tool-id]').forEach((card)=>{const tool=state.tools.find((item)=>item.id===card.dataset.libraryToolId&&!item.archivedAt);if(!tool)return;const existing=card.querySelector('[data-tool-usage]');const usage=state.treatmentComponents.filter((component)=>component.toolId===tool.id).length;if(!usage){existing?.remove();return;}if(!existing){const note=document.createElement('p');note.className='muted';note.dataset.toolUsage='true';note.textContent=`Usado em ${usage} ${usage===1?'componente':'componentes'} de tratamento.`;card.appendChild(note);}else{existing.textContent=`Usado em ${usage} ${usage===1?'componente':'componentes'} de tratamento.`;}});}
