...
function fmt(iso){return iso?new Intl.DateTimeFormat('pt-BR',{dateStyle:'long'}).format(new Date(iso)):'—';}
function label(value){return status[value]||value||'';}
...
function reportHtml(state,sessionId,assistedId){
  ...
  const notes=(state.events||[]).filter((e)=>e.sessionId===sessionId&&e.assistedEntityId===assistedId&&e.eventType==='NOTE_CREATED');
  ...
  <section><h2>Tratamentos</h2>${treatments.length?treatments.map((t)=>`<article><h3>${esc(t.title)}</h3>${t.therapeuticObjective?`<p><strong>Objetivo:</strong> ${esc(t.therapeuticObjective)}</p>`:''}<p class="muted">${esc(label(t.status))}</p>${components(t.id).length?`<ul>${components(t.id).map((c)=>`<li><strong>${esc(c.name)}</strong>${c.instructions?` — ${esc(c.instructions)}`:''}${c.expectedEndAt?`<br><span class="muted">Revisão prevista: ${fmt(c.expectedEndAt)}</span>`:''}</li>`).join('')}</ul>`:''}</article>`).join(''):'<p class="muted">Nenhum tratamento foi iniciado ou alterado nesta sessão.</p>'}</section>
  <section><h2>Reiki</h2>${reiki.length?`<ul>${reiki.map((r)=>`<li>${esc(reikiMode[r.mode]||'Aplicação')} · ${esc(label(r.status))}${r.durationSeconds!=null?` · ${Math.round(r.durationSeconds/60)} min`:''}</li>`).join('')}</ul>`:'<p class="muted">Nenhuma aplicação registrada nesta sessão.</p>'}</section>
  ${notes.length?`<section><h2>Orientações / observações</h2><ul>${notes.map((e)=>`<li>${esc(e.metadata?.body||e.metadata?.notes||'')}</li>`).join('')}</ul></section>`:''}
  <p class="muted">Este resumo reúne apenas informações selecionadas da sessão. O histórico técnico completo permanece registrado no Fluxa.</p><script>const b=document.querySelector('#share');if(navigator.share){b.hidden=false;b.onclick=async()=>{try{await navigator.share({title:document.title,text:document.body.innerText.replace('Imprimir / salvar em PDF','').replace('Compartilhar','')});}catch(e){}}}</script></body></html>`;
}
