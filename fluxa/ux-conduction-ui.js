...
function postCloseDialog(sessionId){
  ...
  wrap.innerHTML=`<section class="sheet ux-completion-sheet"><div class="sheet-head"><div><p class="eyebrow">Sessão encerrada</p><h2>Encerramento registrado com segurança</h2></div><button class="close-btn" data-ux-post-close-close>×</button></div><div class="ux-complete-mark">✓</div><p class="muted">A sessão foi fechada. Agora você pode gerar a documentação.</p><div class="ux-post-close-groups"><section><p class="eyebrow">Para mim</p><h3>Registro técnico</h3><button class="btn secondary wide" data-session-report data-session="${esc(sessionId)}">Registro técnico completo</button></section>${ids.size?`<section><p class="eyebrow">Por assistido</p><h3>Relatórios e resumos</h3><div class="ux-post-close-actions">${[...ids].map((id)=>`<button class="btn secondary wide" data-session-report data-session="${esc(sessionId)}" data-assisted="${esc(id)}">Relatório · ${esc(assistedName(state,id))}</button>`).join('')}</div></section>`:''}</div><button class="btn primary wide" data-ux-post-close-close>Voltar para Hoje</button></section>`;
