...
import { resumeRootProtocol } from './legacy-protocol-adapter.js';
...
function openExactInvestigation(inv,session){
  ...
  if(inv.kind==='BRANCHING'){
    resumeBranchingInvestigation(store,inv.id,session.id);document.querySelector('[data-route="today"]')?.click();requestAnimationFrame(()=>{document.querySelector('[data-action="investigate"]')?.click();requestAnimationFrame(()=>clickAfterRender(`[data-start-branching="${CSS.escape(inv.protocolId)}"]`));});return;
  }
  if(inv.kind==='ROOT_PROTOCOL'){
    resumeRootProtocol(inv.id);document.querySelector('[data-route="today"]')?.click();requestAnimationFrame(()=>{document.querySelector('[data-action="investigate"]')?.click();requestAnimationFrame(()=>clickAfterRender(`[data-start-root-protocol="${CSS.escape(inv.protocolId)}"]`));});return;
  }
  if(inv.kind==='CUSTOM_BRANCHING'){
    document.querySelector('[data-route="library"]')?.click();requestAnimationFrame(()=>clickAfterRender(`[data-start-custom-protocol="${CSS.escape(inv.protocolId)}"]`));return;
  }
  resumeInvestigation(store,inv.id,session.id);document.querySelector('[data-route="today"]')?.click();requestAnimationFrame(()=>document.querySelector('[data-action="resume-latest-investigation"]')?.click());
}
