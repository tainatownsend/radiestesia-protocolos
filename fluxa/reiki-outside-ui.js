...
function reikiEnabled() {
  const enabled=store.getState()?.settings?.therapeuticModalities?.enabled;
...
function ensureAction() {
  ...
  if (!reikiEnabled()) { main.querySelector('[data-reiki-outside]')?.remove(); close(); return; }
...
document.addEventListener('click',(event)=>{ const b=event.target.closest('button'); if(!b)return; if(b.dataset.reikiOutsideOpen!==undefined){ if(!reikiEnabled()) return; const current=active(); current?timerDialog(current.id):startDialog(); } else if(b.dataset.reikiOutsideClose!==undefined) close(); else if(b.dataset.reikiOutsidePause){ pauseFlexibleReiki(store,b.dataset.reikiOutsidePause); timerDialog(b.dataset.reikiOutsidePause); } else if(b.dataset.reikiOutsideResume){ try{resumeFlexibleReiki(store,b.dataset.reikiOutsideResume);timerDialog(b.dataset.reikiOutsideResume);}catch(e){alert(e.message);} } else if(b.dataset.reikiOutsideFinish) finishDialog(b.dataset.reikiOutsideFinish); },true);
document.addEventListener('submit',(event)=>{ const form=event.target; if(form.id==='reiki-outside-start'){event.preventDefault();if(!reikiEnabled())return;const d=new FormData(form);try{const app=startFlexibleReiki(store,{assistedEntityId:d.get('assistedEntityId'),mode:d.get('mode')||ReikiMode.OTHER});timerDialog(app.id);}catch(e){alert(e.message);}} else if(form.id==='reiki-outside-finish'){event.preventDefault();const d=new FormData(form);try{completeFlexibleReiki(store,form.dataset.reiki,d.get('notes'));close();}catch(e){alert(e.message);}} },true);
