...
function sessionAssistedIds(state,sessionId){
  ...
  (state.events||[]).filter((e)=>e.sessionId===sessionId).sort((a,b)=>a.occurredAt.localeCompare(b.occurredAt)).forEach((e)=>add(e.assistedEntityId));
...
function activeInvestigation(state,assistedId){return (state.investigations||[]).filter((i)=>i.assistedEntityId===assistedId&&i.status==='IN_PROGRESS').sort((a,b)=>(b.updatedAt||b.createdAt||'').localeCompare(a.updatedAt||a.createdAt||''))[0]||null;}
...
function reviewableTreatment(state,assistedId){
  ...
  return treatments.find((t)=>(state.treatmentComponents||[]).some((c)=>c.treatmentId===t.id&&c.status==='IN_PROGRESS'&&c.expectedEndAt&&new Date(c.expectedEndAt).getTime()<=now))||null;
}
function setFocusMode(enabled){document.body.classList.toggle('fluxa-focus-mode',enabled);sessionStorage.setItem(FOCUS_KEY,enabled?'1':'0');}
...
function ensureFastContext(){
  ...
  bar.innerHTML=`<div class="fast-context-main"><div><p class="eyebrow">Atendimento atual</p><strong>${current?esc(assistedName(state,current)):'Nenhum assistido selecionado'}</strong></div><div class="fast-context-actions"><button class="btn ghost small" data-fast-focus-mode>${document.body.classList.contains('fluxa-focus-mode')?'Mostrar contexto':'Modo atendimento'}</button><button class="btn secondary small" data-action="choose-assisted">${current?'Trocar':'Escolher assistido'}</button></div></div>
    ${ids.length>1?`<div class="fast-assisted-chips">${ids.map((id)=>`<button class="fast-assisted-chip ${id===current?'active':''}" data-fast-assisted="${id}">${esc(assistedName(state,id))}</button>`).join('')}</div>`:''}
    ${current&&(inv||reiki||review)?`<div class="fast-next-actions">${inv?`<button class="fast-next" data-fast-resume-investigation><span>Continuar</span><strong>${esc(protocolName(inv))}</strong></button>`:''}${reiki?`<button class="fast-next" data-fast-open-reiki="${reiki.id}"><span>${reiki.status==='PAUSED'?'Reiki pausado':'Reiki ativo'}</span><strong>Abrir timer</strong></button>`:''}${review?`<button class="fast-next" data-fast-review-treatment="${review.id}"><span>Revisão disponível</span><strong>${esc(review.title)}</strong></button>`:''}</div>`:''}`;
  main.prepend(bar);
}

function enhance(){if(enhancing)return;enhancing=true;try{ensureFastContext();}finally{enhancing=false;}}
setFocusMode(sessionStorage.getItem(FOCUS_KEY)==='1');
