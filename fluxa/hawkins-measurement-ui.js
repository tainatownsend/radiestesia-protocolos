...
function baselineCard({session,assisted,baseline},compact=false,options={}){
  ...
  return `<section class="hawkins-measurement-card ${compact?'compact':''}" data-hawkins-baseline-card><div><p class="eyebrow">Medição inicial obrigatória</p><strong>Frequência vibracional de Hawkins</strong><p>Registre a frequência em Hz antes de iniciar a investigação ou o tratamento.</p></div><form data-hawkins-baseline-form data-session="${esc(session.id)}" data-assisted="${esc(assisted.id)}"${link}${resume}${resumeLegacy}><label><span>Frequência</span><span class="hawkins-input"><input name="hertz" type="number" min="0.01" step="any" inputmode="decimal" required placeholder="Ex.: 540"><b>Hz</b></span></label><button class="btn primary" type="submit">Registrar</button></form></section>`;
}
