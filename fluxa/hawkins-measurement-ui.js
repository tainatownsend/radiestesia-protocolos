...
const store=createStore();let enhancing=false;let pendingTreatmentLink=null;let pendingFinalLink=null;let pendingLegacyInvestigationButton=null;
...
function baselineCard({session,assisted,baseline},compact=false,options={}){
  ...
  const link=options.treatmentId?` data-link-treatment="${esc(options.treatmentId)}"`:'';const resume=options.resumePlanned?` data-resume-planned-start="${esc(options.treatmentId)}"`:'';const resumeLegacy=options.resumeLegacyInvestigation?' data-resume-legacy-investigation="true"':'';
  return `<section class="hawkins-measurement-card ${compact?'compact':''}" data-hawkins-baseline-card><div><p class="eyebrow">Antes de começar com este Assistido</p><strong>Frequência vibracional de Hawkins</strong><p>Meça ${esc(assisted.displayName||'o Assistido')} agora. Este será o valor inicial para comparar com a medição feita ao final do tratamento.</p></div><form data-hawkins-baseline-form data-session="${esc(session.id)}" data-assisted="${esc(assisted.id)}"${link}${resume}${resumeLegacy}><label><span>Frequência inicial</span><span class="hawkins-input"><input name="hertz" type="number" min="0.01" step="any" inputmode="decimal" required placeholder="Ex.: 350"><b>Hz</b></span></label><button class="btn primary" type="submit">Registrar e continuar</button></form></section>`;
}
...
function enhanceFinalForm(){
  const form=document.querySelector('#final-assessment-form,#final-cycle-form');if(!form||form.dataset.hawkinsEnhanced)return;form.dataset.hawkinsEnhanced='true';const input=form.querySelector('[name="frequency"]');if(!input)return;input.type='number';input.min='0.01';input.step='any';input.inputMode='decimal';input.placeholder='Ex.: 540';const field=input.closest('.field');const label=field?.querySelector('label');if(label)label.textContent='Frequência vibracional de Hawkins (Hz) · medição final';field?.querySelector('.hawkins-helper')?.remove();const helper=document.createElement('small');helper.className='muted hawkins-helper';helper.textContent='Meça novamente o Assistido para comparar com a frequência inicial e registrar a evolução global.';field?.appendChild(helper);
}
...
function openLegacyInvestigationBaseline(button,ctx){
  pendingLegacyInvestigationButton=button;document.querySelector('#hawkins-investigation-overlay')?.remove();const wrap=document.createElement('div');wrap.id='hawkins-investigation-overlay';wrap.className='modal-backdrop';wrap.innerHTML=`<section class="sheet"><div class="sheet-head"><div><p class="eyebrow">Antes de investigar</p><h2>Frequência inicial do Assistido</h2></div><button class="close-btn" type="button" data-close-hawkins-investigation>×</button></div>${baselineCard(ctx,true,{resumeLegacyInvestigation:true})}</section>`;document.body.appendChild(wrap);requestAnimationFrame(()=>wrap.querySelector('[name="hertz"]')?.focus());
}
...
document.addEventListener('submit',event=>{
  ...
  if(form.matches('[data-hawkins-baseline-form]')){event.preventDefault();event.stopImmediatePropagation();try{const data=new FormData(form),assessment=recordHawkinsBaseline(store,{sessionId:form.dataset.session,assistedEntityId:form.dataset.assisted,hertz:data.get('hertz')});if(form.dataset.linkTreatment)linkTreatmentHawkinsBaseline(store,form.dataset.linkTreatment,assessment.id);const resumeId=form.dataset.resumePlannedStart;const resumeLegacy=form.dataset.resumeLegacyInvestigation==='true';form.closest('[data-hawkins-baseline-card]')?.remove();document.querySelector('#hawkins-planned-overlay')?.remove();document.querySelector('#hawkins-investigation-overlay')?.remove();queueMicrotask(enhance);if(resumeId)requestAnimationFrame(()=>document.querySelector(`[data-start-planned-treatment="${CSS.escape(resumeId)}"]`)?.click());if(resumeLegacy){const button=pendingLegacyInvestigationButton;pendingLegacyInvestigationButton=null;if(button)requestAnimationFrame(()=>button.click());}}catch(error){alert(error.message);}return;}
...
document.addEventListener('click',event=>{
  ...
  if(button.dataset.closeHawkinsInvestigation!==undefined){pendingLegacyInvestigationButton=null;document.querySelector('#hawkins-investigation-overlay')?.remove();return;}
  ...
  if(button.dataset.action==='investigate'&&!button.closest('#investigation-chooser-overlay')){const ctx=context();if(!ctx.session||!ctx.assisted||latestPreparation(ctx.state,ctx.session.id)?.status!=='COMPLETED'||ctx.baseline)return;event.preventDefault();event.stopImmediatePropagation();openLegacyInvestigationBaseline(button,ctx);return;}
