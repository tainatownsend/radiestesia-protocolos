...
function buildCockpit(state,session){
  ...
  let label='Próximo passo',title='Escolha a atividade',detail='Investigue primeiro ou siga para a composição de um tratamento radiestésico.',action='';
  ...
  return `<section class="home-cockpit" data-home-cockpit>
    ...
    <nav class="home-primary-actions" data-home-actions aria-label="Ações principais da sessão">
      ...
      <button class="home-action" data-action="treat-direct" ${id?'':'disabled'}>${actionIcon('treat')}<strong>Compor tratamento</strong><span>Radiestesia + terapias opcionais</span></button>
...
function simplifySessionTimeline(main){
  ...
  const btn=document.createElement('button');btn.type='button';btn.className='btn ghost small';btn.dataset.homeToggleSection='true';btn.textContent='Atividade';head.appendChild(btn);
  const close=head.querySelector('[data-action="close-session"]');if(close){close.textContent='Encerrar';close.classList.add('home-close-session');head.appendChild(close);}
}
function simplifyIdle(main){
  ...
  const reikiButton=main.querySelector('[data-action="reiki-retro"]')?.closest('.section');if(reikiButton)reikiButton.hidden=true;
}
