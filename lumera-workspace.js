...
function saveFlows(arr){localStorage.setItem(FLOW_STORE,JSON.stringify(arr.slice(0,100)))}
...
function activeAssessment(){
 ...
 const all=parse(ASSESSMENT_STORE);return all.find(x=>x.status==='draft')||latest(all);
}
function personLabel(a){
 if(!a)return 'Nenhuma sessão ativa';
 const id=a.identity||{};
...
function stageData(){
 ...
 let treatment=false, report=false, reeval=false, investigation=false;
 if(a){
   investigation=!!a.linkedSessionId;
   const s=sessions.find(x=>String(x.id)===String(a.linkedSessionId));
   if(s){treatment=(s.treatment||[]).length>0;report=!!s.final;reeval=!!a.reevaluatedAt}
 }
 const d=latest(divorce);if(d&&d.status==='completed'){treatment=true;report=true}
 return {a,recentPrep,recentClose,investigation,treatment,reeval,report};
}
function ensureOverlay(){
 if($('practitionerView'))return;
 const v=document.createElement('div');v.id='practitionerView';v.className='practitionerView hidden';v.innerHTML='<div class="practitionerPanel"><div id="practitionerContent"></div></div>';document.body.appendChild(v);
}
...
function openFlow(type){
 ...
 $('practiceForm').onsubmit=e=>{e.preventDefault();const checks=Array.from(e.currentTarget.querySelectorAll('input[type=checkbox]'));const item={id:Date.now(),type,date:day(),createdAt:stamp(),completed:checks.filter(x=>x.checked).length,total:checks.length,notes:$('practiceNotes').value||''};const arr=flows();arr.unshift(item);saveFlows(arr);closeOverlay();renderWorkspace();if(type==='prepare'){const btn=$('startAssessmentBtn');if(btn)btn.focus()}}
}
function closeOverlay(){$('practitionerView')?.classList.add('hidden');document.body.style.overflow=''}
function dashboardHTML(){
 const d=stageData(),a=d.a,stages=[['Preparação',!!d.recentPrep],['Avaliação',!!a],['Investigação',d.investigation],['Tratamento',d.treatment],['Relatório',d.report]];
 let active=stages.findIndex(x=>!x[1]);if(active<0)active=4;
 const life=a?.lifeAreas||{};const vals=Object.entries(life).filter(([,v])=>v!==''&&v!=null).map(([k,v])=>[k,Number(v)]).sort((x,y)=>y[1]-x[1]);
 const names={family:'Familiar',affective:'Relacionamento',professional:'Profissional',financial:'Financeiro',mission:'Missão de vida'};
 const worst=vals[0];const vib=a?.vibration?.hz?`${a.vibration.hz} Hz${a.vibration.label?' • '+a.vibration.label:''}`:'Não aferida';
 return `<section class="sessionDashboard"><div class="sessionDashboardHead"><div><h3>${a?'Sessão em andamento':'Dashboard da sessão'}</h3><p>${personLabel(a)}</p></div><span class="sessionStatus">${d.recentClose?'Encerrada hoje':a?'Em andamento':'Pronto para iniciar'}</span></div><div class="sessionTimeline">${stages.map((s,i)=>`<div class="sessionStage ${s[1]?'done':i===active?'active':''}">${s[1]?'✓ ':''}${s[0]}</div>`).join('')}</div><div class="dashboardFacts"><div class="dashboardFact"><small>Frequência vibracional</small><strong>${vib}</strong></div><div class="dashboardFact"><small>Área que mais pede atenção</small><strong>${worst?(names[worst[0]]||worst[0])+' • '+worst[1]+'%':'Ainda não aferida'}</strong></div><div class="dashboardFact"><small>Próximo passo</small><strong>${!d.recentPrep?'Preparar sessão':!a?'Avaliação inicial':!d.investigation?'Escolher investigação':!d.treatment?'Realizar tratamento':!d.report?'Concluir e gerar relatório':'Encerrar sessão'}</strong></div></div></section>`;
}
...
function renderWorkspace(){
 const home=$('homeView');if(!home)return;let w=home.querySelector('.lumeraWorkspace');if(!w){w=document.createElement('div');w.className='lumeraWorkspace';const intro=home.querySelector('.homeIntro');intro?.after(w)}w.outerHTML=workspaceHTML();bindWorkspace();
}
function bindWorkspace(){
 $('prepareSessionBtn')?.addEventListener('click',()=>openFlow('prepare'));
 $('closeSessionBtn')?.addEventListener('click',()=>openFlow('close'));
 $('jumpAssessmentBtn')?.addEventListener('click',()=>{const b=$('startAssessmentBtn');if(b)b.click();else document.querySelector('.sessionLauncher')?.scrollIntoView({behavior:'smooth',block:'start'})});
}
function decorateBrand(){
 document.title='Lumera — Radiestesia Terapêutica';const b=$('headerHomeBtn');if(b){b.textContent='Lumera';b.setAttribute('aria-label','Voltar ao início do Lumera')}
...
function addClosePrompt(){
 const report=$('reportView');if(!report||report.classList.contains('hidden')||report.querySelector('.reportClosePrompt'))return;
 const actions=report.querySelector('.noPrint')||report.lastElementChild;if(!actions)return;const box=document.createElement('div');box.className='reportClosePrompt noPrint';box.innerHTML='<b>Antes de finalizar</b><p>Use o encerramento guiado para fechar o campo de trabalho, registrar pendências e concluir a sessão.</p><button type="button" class="ghost wide" id="reportCloseSession">Encerrar sessão</button>';actions.prepend(box);$('reportCloseSession').onclick=()=>openFlow('close');
}
function observe(){
 const observer=new MutationObserver(()=>{decorateBrand();if($('homeView')&&!$('homeView').classList.contains('hidden'))renderWorkspace();addClosePrompt()});observer.observe(document.body,{subtree:true,attributes:true,attributeFilter:['class']});
 window.addEventListener('storage',()=>renderWorkspace());document.addEventListener('visibilitychange',()=>{if(!document.hidden)renderWorkspace()});
}
